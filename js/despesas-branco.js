// despesas-branco.js - Caminhão Branco Multi-Tenant
// Sistema de Postes com suporte específico para Caminhão Branco
...
async function initDespesas() {
    console.log('🎯 Inicializando Despesas Caminhão Branco...');
    ...
    try {
        ...
        await loadData();
        console.log('✅ Despesas Caminhão Branco carregado');
    } catch (error) {
    ...
    if (!despesas || despesas.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                ...
                <p>Comece cadastrando sua primeira despesa do Caminhão Branco.</p>
...
function updateResumo() {
    ...
    if (resumoTitle) {
        ...
        if (hasFilters) {
            resumoTitle.textContent = '📊 Resumo das Despesas (Filtrado) - Caminhão Branco';
        } else {
            resumoTitle.textContent = '📊 Resumo das Despesas - Caminhão Branco';
        }
    }
}
...
async function exportarDespesas() {
    ...
    window.AppUtils.exportToCSV(dadosExportar, `despesas_branco_${suffix}_${new Date().toISOString().split('T')[0]}`);
}
