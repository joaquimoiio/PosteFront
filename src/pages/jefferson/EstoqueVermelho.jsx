...
export default function EstoqueVermelho() {
  const { data, loading, error } = useEstoque();

  const vermelhoData = data.filter(e => !e.tenantId || e.tenantId === 'vermelho');
  ...
  const displayData = vermelhoData.length > 0 ? vermelhoData : data;

  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
          <Truck size={20} className="text-red-600" /> Estoque Vermelho
        </h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">Itens do caminhão vermelho</p>
      </div>
